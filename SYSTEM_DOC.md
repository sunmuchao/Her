...
当前系统同时承载推荐、撮合、聊天、审核、认证、风控、登录、AI 会话。优势是闭环完整，风险是复杂度快速上升。
...
后续若继续扩展，可能需要更明确的 API 分层或后台 API 分拆。

### 13.5 规则系统仍以代码常量为主

如搜索、推荐 gate、风险阈值、quiet hours 等多以代码和环境变量配置，后续若需要运营可调，会需要规则配置平台。
