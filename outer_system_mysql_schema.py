...
def chat_tables() -> tuple[TableDef, ...]:
    ...
    Now delegates to chat_tables_core() for backward compatibility.
    """
    return chat_tables_core()
...
def infrastructure_tables() -> tuple[TableDef, ...]:
    """Infrastructure tables: shared outbox_events and async_jobs."""
    return (
        TableDef(
            name="outbox_events",
            columns=(
...
def auth_tables() -> tuple[TableDef, ...]:
    """Authentication account tables (9 tables)."""
    return (
        TableDef(
            name="user_accounts",
            columns=(
...
def verification_tables() -> tuple[TableDef, ...]:
    """Verification review tables (12 tables)."""
    return (
        TableDef(
...
def risk_tables() -> tuple[TableDef, ...]:
    """Risk control tables (17 tables)."""
    return (
        TableDef(
...
def chat_tables_core() -> tuple[TableDef, ...]:
    """Core chat tables (8 tables) + persona_sync_jobs + call_sessions."""
    return (
        TableDef(
            name="chat_threads",
            columns=(
