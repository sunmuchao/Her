...
from recommendation_system.outbox import recover_stale_outbox_claims  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage recommendation outbox consumption and retry queues.")
    parser.add_argument(
        "command",
        choices=(
            "summary",
    ...
    parser.add_argument("--dsn", default=DEFAULT_RECOMMENDATION_MYSQL_DSN)
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "summary":
            ...
            payload = run_recommendation_outbox_worker(
                conn,
                limit=args.limit,
            ...
            payload = serve_recommendation_outbox_worker(
                conn,
                limit=args.limit,
            ...
            recovered = recover_stale_outbox_claims(
                ...
                error_message="recommendation outbox processing timed out",
            )
            conn.commit()
            payload = {
                "recovered": recovered,
        ...
        audit_cli_action(
            args,
            action=f"recommendation_outbox.{args.command}",
            resource_type="recommendation_outbox",
