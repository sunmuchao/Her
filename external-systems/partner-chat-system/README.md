...
- **默认 `--assistant-mode proactive`**：每轮发言前先走一层**规则快路径**（明显冷场、重复敷衍、明显正常可继续时不再多调一次模型），模糊情况再用**调度模型**只看「双方可见」记录判断是否冷场/尬聊；需要时再调 **`assistant_query`** 给**即将开口的那一方**私下指出问题并给出接话建议。局面明显难救时，助手现在也会给**体面收口**建议，而不是一味硬救。
...
- 输出 JSON 额外包含 `turn_evaluations[].assistant_latency_ms`、`turn_evaluations[].message_generation_source`、`assistant_metrics.assistant_invoke_avg_ms`、`assistant_metrics.assistant_invoke_max_ms`、`assistant_metrics.heuristic_decision_turns`、`assistant_metrics.fallback_message_turns`、`assistant_metrics.llm_error_fallback_turns`、`assistant_metrics.graceful_exit_advice_turns`、`llm_stats`，便于排查“到底是调度慢、主回复慢，还是助手建议慢”，以及本轮有没有走降级或止损。  
