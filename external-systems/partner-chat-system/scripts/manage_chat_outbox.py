...
from chat_system.storage import DEFAULT_CHAT_MYSQL_DSN, connect_db, initialize_database  # noqa: E402
from chat_system.outbox_admin import recover_stale_outbox_claims  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage chat outbox consumption and retry queues.")
    parser.add_argument(
        "command",
        choices=(
            "summary",
    ...
    parser.add_argument("--dsn", default=DEFAULT_CHAT_MYSQL_DSN)
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "summary":
            payload = summarize_outbox(conn, now=now, claim_timeout_seconds=args.claim_timeout_seconds or 300)
        elif args.command == "consume":
            payload = run_chat_outbox_worker(
                conn,
                limit=args.limit,
            ...
            payload = serve_chat_outbox_worker(
                conn,
                limit=args.limit,
            ...
            payload = {
                ...
                "summary": summarize_outbox(conn, now=now, claim_timeout_seconds=args.claim_timeout_seconds or 300),
            }
        ...
        audit_cli_action(
            args,
            action=f"chat_outbox.{args.command}",
            resource_type="chat_outbox",
