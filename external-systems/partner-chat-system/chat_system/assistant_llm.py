...
def _humanize_interaction_mode(value: str) -> str:
    ...
    return mapping.get(value, mapping["none"])
...
def _finalize_guidance_with_profile_context(
    ...
    *,
    ...
    if interaction_mode != "repair":
        out["topic_directions"] = []
        out["profile_hooks_used"] = []
        return out
    ...
    if not hooks_used and selected_hooks:
...
def build_placeholder_assistant_guidance(
    *,
    ...
    guidance = {
        ...
        "avoid": _default_avoid(normalized_mutual_intent, normalized_interaction_mode),
...
def generate_assistant_guidance(
    *,
    ...
    if preferred_interaction_mode_normalized != "repair" and (
        preferred_interaction_mode is not None or preferred_mutual_intent_assessment is not None
    ):
        return fallback
    ...
    if route_reason or engagement_level or warmth_level or irritation_level or state_trend:
        prompt_parts.append(
            "快照: "
