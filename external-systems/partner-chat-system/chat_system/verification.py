...
def _normalize_machine_score(value: Any, default: int) -> int:
    try:
...
def _is_photo_review_signal_list(signal_codes: list[str] | None) -> bool:
...
def _photo_review_task_metadata(
    *,
    request_source: str,
...
def _merge_photo_review_task_metadata(
    ...
    *,
    ...
    incoming = _photo_review_task_metadata(
...
def _photo_review_notification_copy(
...
def _score_result(score: int, *, pass_threshold: int = 85, fail_threshold: int = 60) -> str:
...
def _challenge_phrase(required_actions: list[str], spoken_code: str | None = None) -> str:
...
def _decode_live_challenge_token(challenge_token: str) -> dict[str, Any]:
    ...
    payload["required_actions"] = _normalize_action_keys(payload.get("required_actions"))
    return payload
...
def create_live_video_verification_challenge(
    *,
    ...
    if not required_actions:
        ...
        required_actions = _choose_default_challenge_actions(
    ...
    phrase = _challenge_phrase(required_actions, spoken_code)
    prompt_steps = _challenge_prompt_steps(required_actions, spoken_code)
    expires_at = _current_time(ts).replace(microsecond=0)
    expires_at = datetime.fromtimestamp(expires_at.timestamp() + _challenge_ttl_seconds())
...
def _validate_live_challenge_submission(
    *,
    ...
    payload = _decode_live_challenge_token(challenge_token)
...
def list_verification_notifications(
    ...
    *,
    ...
    normalized_types = _parse_statuses(notification_types)
    if normalized_types:
...
def _create_verification_notification(
    conn,
    *,
    submission_id: str,
    user_id: str,
...
def _maybe_record_submission_notification(
    ...
    *,
    ...
    if _notification_already_recorded(
        ...
        return
    title, body = _photo_review_notification_copy(notification_type, submission, note=note)
...
def request_live_video_verification(
    ...
    *,
    ...
    if not _is_photo_review_signal_list(normalized_signal_codes):
    ...
    if existing:
        ...
        merged_metadata[PHOTO_REVIEW_METADATA_KEY] = _merge_photo_review_task_metadata(
        ...
        merged_metadata = _append_workflow_history(
            merged_metadata,
    ...
    task_metadata = _photo_review_task_metadata(
        request_source=request_source,
        signal_codes=normalized_signal_codes,
        request_reason=request_reason,
        requested_by=requested_by,
    ...
    submission_metadata = _append_workflow_history(
        submission_metadata,
...
def _derive_action_challenge_score(
    *,
...
def _apply_speech_review_to_machine_scores(
    *,
    challenge_score: int,
    risk_flags: list[str],
    ...
    speech_score = _normalize_machine_score(speech_review.get("speech_score"), 0)
...
def review_live_video_verification(
    ...
    *,
    ...
    if _normalize_metadata(merged_submission_metadata.get(PHOTO_REVIEW_METADATA_KEY)).get("task_kind") == "photo_review":
        ...
        merged_submission_metadata = _append_workflow_history(
