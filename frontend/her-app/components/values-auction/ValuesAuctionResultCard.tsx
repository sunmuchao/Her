...
import { Share2, Eye, ArrowRight, Check, Sparkles } from 'lucide-react'
...
export function ValuesAuctionResultCardComponent({ card, onViewInterpretation, onShare, onContinue }: Props) {
  ...
  }, [revealedCount, isAutoRevealing, top3.length])
        ...
        <div className="relative flex justify-center mb-3">
          <div className="relative will-change-transform">
            ...
            <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-amber-soft to-gold-soft">
              <Sparkles className="w-7 h-7 text-amber animate-heartbeat" />
            </div>
          </div>
        </div>
        <h2 className="text-xl font-semibold">{"拍卖完成！"}</h2>
      </div>
        ...
        <div className="space-y-2.5">
          {top3.slice(0, revealedCount).map((lot, i) => (
            <div
              key={lot.lot_id}
              className={cn(
                'rounded-xl p-3 border animate-fade-in-up will-change-transform',
