    ...
    return {
        ...
        "recent_timeline_summary": [
            ...
            {"item_type": "assistant_message", "body": "好的，我按稳定职业方向帮你看几位。"},
            {
                ...
                "title": "先看这几位",
                "cards": [
                    {"profile_id": 573, "title": "陈雨桐 30", "reason_summary": "工作稳定，作息规律"},
                    {"profile_id": 6609, "title": "周可心 32", "reason_summary": "情绪稳定，长期定居"},
                ],
            },
        ],
        "visible_actions": [
...
def _build_run_input(runtime_context: dict[str, Any]) -> DiscoveryRunInput:
    return DiscoveryRunInput(
        ...
        phase="results_shown",
        criteria_labels=["苏州", "女", "26-36岁", "先谈恋爱"],
        recent_timeline=list(runtime_context.get("recent_timeline_summary") or []),
