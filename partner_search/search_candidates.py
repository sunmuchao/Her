...
from match_domain.appearance_features import (
    ...
    refresh_profile_photo_features as _refresh_profile_photo_features,
)
...
RISK_FLAG_PENALTIES = {
    ...
    "当前城市未命中，但定居城市命中": 4,
    "学历略低于你的理想值，但仍然接近": 4,
    "学历有一定差距，需要看实际沟通和认知": 7,
    "学历差距较大": 10,
    "对方年龄要求接近命中，可作为兼容匹配": 4,
    "对方年龄要求有一定偏差，但仍可尝试": 7,
    "对方身高要求接近命中，可作为兼容匹配": 3,
    "对方身高要求有一定偏差，但仍可尝试": 5,
    "对方学历要求接近命中，可作为兼容匹配": 4,
    "对方学历要求有一定差距，但仍可尝试": 6,
    "对方婚史接受度偏保守": 9,
    "对方婚史接受需要先聊再判断": 9,
    "对方婚史接受度未知": 10,
...
def attach_photo_features(records, *, persona_source=None):
    ...
    if missing_ids:
        for candidate_id in missing_ids[:20]:
            try:
                _refresh_profile_photo_features(
                    source_dsn=normalized_source,
