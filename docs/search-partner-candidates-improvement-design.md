...
> **核心思路**：硬约束筛选 → 开放候选人信息给 Agent → Agent 自主分析筛选
...
Step 1: 硬约束筛选
        ...
        → SQL WHERE 子句查询
        → 返回候选人大池子（20-50 人）
        → 只返回 ID + 基础信息（年龄、城市、职业）

Step 2: Agent 选择性加载候选人完整信息
        → Agent 决定要看哪些候选人
        → Agent 决定要看哪些信息类型（persona、chat、behavior）
        → 调用 load_candidate_full_info 工具

Step 3: Agent 自主分析判断
        → Agent 读候选人信息
        → Agent 根据语义理解判断（纯思考，不调用工具）
        → "这个人聊天里过于迎合，有绿茶特征" → exclude
...
| 原则 | 描述 | 原因 |
|------|------|------|
| **Agent 是决策大脑** | Agent 自主决定如何分析筛选 | Agent Native 设计 |
| **硬约束只筛选** | 只返回 ID + 基础信息 | 避免信息量过大 |
| **Agent 选择性读取** | Agent 决定要不要深入看 | 减少不必要的信息读取 |
| **判断是纯思考** | Agent 判断不调用工具 | 不硬编码规则 |
| **信息分层** | persona → chat_summary → chat_raw | 渐进式深入 |
